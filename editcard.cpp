...
Edit_card::Edit_card(QWidget * parent) : QWidget(parent){
    setFixedWidth(500);
    ...
    save_button = new QPushButton("Сохранить изменения");
    ...
    common_layout->addWidget(save_button);


    this->setLayout(common_layout);

    connect(button_image_path, &QPushButton::clicked, this, &Edit_card::on_button_image_path_clicked);
    connect(save_button, &QPushButton::clicked, this, &Edit_card::on_save_button_clicked);
}

void Edit_card::on_save_button_clicked()

{
    if (line_edit_first_name->text().size() == 0 ||
        ...
        emit save_button_clicked_signal(employee);
    }
}

void Edit_card::on_get_employee(Employee employee)
{
    line_edit_second_name->setText(employee.get_second_name());
    line_edit_first_name->setText(employee.get_first_name());
    line_edit_third_name->setText(employee.get_third_name());
    line_edit_titul->setText(employee.get_titul());
    edit_birth_date->set_date(employee.get_birth_date());
    edit_company_start_date->set_date(employee.get_company_start_date());
    spin_box_experience->setValue(employee.get_work_experience());
    line_edit_job_title->setText(employee.get_job_title());
    line_edit_salary->setText(QString::number(employee.get_salary()));
    QPixmap image(100, 100);
    if(image.load(employee.get_image_path()))
    {
        label_image_path->setText(employee.get_image_path());
...
void Edit_card::on_button_image_path_clicked()
{
